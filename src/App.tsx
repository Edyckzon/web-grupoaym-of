...
import { SolutionsExplorer, GettingStarted, FrequentlyAsked, LocationSection, GroupApproach } from './CompanyContent'
import './App.css'
  ...
  return <>
    ...
    <header className="header"><a className="home-link" href="/" onClick={navigate} aria-label="Grupo AyM, inicio"><Wordmark /></a>
      <nav id="navigation" className={menuOpen ? 'open' : ''} aria-label="Navegación principal"><a href="/" onClick={navigate} aria-current={path === '/' ? 'page' : undefined}>Inicio</a>
        ...
        <a href="/#soluciones" onClick={navigate}>Soluciones</a><a href="/nosotros" onClick={navigate} aria-current={path === '/nosotros' ? 'page' : undefined}>Nosotros</a><a href="/contacto" onClick={navigate} aria-current={path === '/contacto' ? 'page' : undefined}>Contacto</a></nav>
    ...
    <footer className="footer"><div className="footer-grid"><div className="footer-about"><a href="/" onClick={navigate} aria-label="Grupo AyM, inicio"><Wordmark /></a><p>Contabilidad · Tecnología · Movilidad</p><p>Tres marcas conectadas para acompañar el crecimiento de empresas y personas.</p></div><div><h2>Nuestras marcas</h2>{companies.map((c, i) => c.destinations.length === 1 ? <BrandLink key={c.id} company={c} destination={c.destinations[0]} placement="footer">{c.name}</BrandLink> : <button key={c.id} onClick={e => openDetail(i, e)} aria-haspopup="dialog">{c.name}</button>)}</div><div><h2>El grupo</h2><a href="/nosotros" onClick={navigate}>Nuestra historia</a><a href="/#ecosistema" onClick={navigate}>Nuestro ecosistema</a><a href="/#soluciones" onClick={navigate}>Soluciones</a><a href="/#preguntas" onClick={navigate}>Preguntas frecuentes</a><a href="/contacto" onClick={navigate}>Contacto</a></div><div><h2>Conversemos</h2><p>Encuentra los canales de atención de nuestras marcas.</p><a className="footer-cta" href="/contacto" onClick={navigate}>Contáctanos <Arrow /></a>{contact.address && <a href="/contacto#ubicacion" onClick={navigate}>{contact.address}</a>}<a href={`tel:${contact.landlineHref}`} onClick={() => trackEvent('phone_click', { placement: 'footer_central' })}><span className="footer-phone-label">Central</span>{contact.landline}</a>{contact.hours && <p>{contact.hours}</p>}{contact.email && <a href={`mailto:${contact.email}`} onClick={() => trackEvent('email_click', { placement: 'footer' })}>{contact.email}</a>}{contact.phone && <a href={`tel:${contact.phone.replace(/[^+\d]/g, '')}`} onClick={() => trackEvent('phone_click', { placement: 'footer' })}>{contact.phone}</a>}</div></div><div className="footer-bottom"><span>© {new Date().getFullYear()} Grupo AyM. Todos los derechos reservados.</span><span>Perú</span></div></footer>
